...
A small dependency-free API console — create/list/delete Stores, Products and Warehouses,
associate Warehouse-Product-Store fulfillment, and see every request/response (including
validation errors like a duplicate business unit code) in a live request log on the same
page.

### API docs / Swagger UI

All endpoints (hand-coded Store/Product and generated Warehouse) are documented and
testable from one place:

<http://localhost:8080/q/swagger-ui/>

See [docs/screenshots](docs/screenshots) for a walkthrough of the Swagger UI, the web UI,
and live examples of validation responses (400/404/422) from the running app.

### Test coverage

See [docs/coverage-report](docs/coverage-report) for a tracked snapshot of the JaCoCo
report (94% line coverage, above the 80% minimum enforced in `pom.xml`). Open
`docs/coverage-report/index.html` in a browser, or run `mvn clean verify` to regenerate a
fresh one at `target/jacoco-report/`.
